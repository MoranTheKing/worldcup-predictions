...
- Scoring lives in `lib/game/scoring.ts`: odds-tier base points + stage points + hit points, then x2 Joker. Persist scores through `scoreFinishedMatchPredictions` so `predictions.points_earned` and `profiles.total_score` stay aligned.
