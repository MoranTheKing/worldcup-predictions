...
function deriveGroupMetrics(matches: TournamentMatch[]) {
  const metrics = new Map<number, { points: number; goalsFor: number; goalsAgainst: number }>();
  ...
  for (const match of matches) {
    ...
    const home = metrics.get(match.home_team_id) ?? { points: 0, goalsFor: 0, goalsAgainst: 0 };
    const away = metrics.get(match.away_team_id) ?? { points: 0, goalsFor: 0, goalsAgainst: 0 };
...
export default async function TournamentPage() {
  ...
  const fallbackTeamsQuery = fullTeamsQuery.error
    ? await supabase
        .from("teams")
        .select("id, name, name_he, logo_url, group_letter, is_eliminated")
        .order("group_letter")
        .order("name_he", { ascending: true })
    : null;
