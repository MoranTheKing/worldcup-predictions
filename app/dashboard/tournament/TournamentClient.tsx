...
const STATUS_META: Record<TeamStanding["status"], { label: string; className: string }> = {
  qualified: {
    label: "עלתה אוטומטית",
    className: "bg-[rgba(95,255,123,0.14)] text-wc-neon",
  },
  third_qualified: {
    label: "עולה מהמקום השלישי",
    className: "bg-[rgba(255,182,73,0.16)] text-wc-amber",
  },
  third_pending: {
    label: "במאבק למקום השלישי",
    className: "bg-white/8 text-wc-fg2",
  },
  third_eliminated: {
    label: "מקום שלישי מודח",
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_auto] lg:items-end">
          <div className="text-start">
            ...
            <p className="mt-4 max-w-3xl text-sm leading-7 text-wc-fg2">
              הטבלאות מסודרות לפי שוברי השוויון הרשמיים של פיפ״א: ראש בראש, הפרש שערים, שערי
              זכות, פייר פליי ודירוג פיפ״א. הנתונים הכוללים נשמרים כ-cache על הנבחרות, וה-Head
              to Head מחושב רק על המשחקים הרלוונטיים של השוויון.
            </p>
          </div>
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            ...
            <HeaderStat label="עולות מהמקום ה-3" value={`${qualifiedThirdCount}/8`} accent="text-wc-amber" />
          </div>
        </div>
      </section>
        ...
        <>
          <div className="mt-6 flex flex-wrap gap-3 text-xs">
            <div className="wc-badge text-wc-fg2">
              ...
              <span>מקומות 1–2 עולות אוטומטית</span>
            </div>
            <div className="wc-badge text-wc-fg2">
              <div className="h-3 w-3 rounded-full bg-wc-amber shadow-[0_0_12px_rgba(255,182,73,0.7)]" />
              <span>המקום השלישי מסומן לטבלת המאבק</span>
            </div>
            <div className="wc-badge text-wc-fg2">
              <div className="h-3 w-3 rounded-full bg-white/25" />
              <span>המקום הרביעי מודח</span>
            </div>
          </div>

          <div className="mt-4 rounded-[1.5rem] border border-white/10 bg-white/4 p-4 text-sm leading-7 text-wc-fg2">
            במקרה של שוויון, פיפ״א בודקת קודם את המאזן הישיר בין הנבחרות השוות. אם עדיין אין
            הכרעה, עוברים להפרש שערים ושערי זכות כלליים, אחר כך לניקוד Fair Play ולבסוף לדירוג
            הרשמי.
          </div>
      ...
      {tab === "third" && (
        <>
          <div className="mt-6 rounded-[1.5rem] border border-[rgba(255,182,73,0.22)] bg-[linear-gradient(135deg,rgba(255,182,73,0.12),rgba(111,60,255,0.08))] p-4 text-sm leading-7 text-[rgba(255,205,136,0.96)]">
            טבלת המקום השלישי מסודרת לפי: נקודות, הפרש שערים כללי, שערי זכות, ניקוד פייר פליי
            ולבסוף דירוג פיפ״א. אין כאן ראש בראש כי הנבחרות מגיעות מבתים שונים.
          </div>
          ...
          <div className="mt-6 overflow-hidden rounded-[1.75rem] wc-card">
            <div className="bg-[linear-gradient(135deg,rgba(95,255,123,0.24),rgba(95,255,123,0.08))] px-4 py-3">
              <p className="text-sm font-bold text-wc-fg1">עולות לשלב 32 האחרונות ({qualifiedThirdCount} מקומות)</p>
            </div>
            ...
            <div className="bg-[linear-gradient(135deg,rgba(255,92,130,0.26),rgba(255,92,130,0.08))] px-4 py-3">
              <p className="text-sm font-bold text-wc-fg1">נפלטות</p>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <BestThirdHead />
                <tbody>
                  {bestThirdStandings
                    .filter((entry) => entry.status === "third_eliminated")
                    .map((entry) => (
                      <BestThirdRow key={entry.team.id} entry={entry} />
                    ))}
                </tbody>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {standings.map((entry) => {
              ...
              return (
                <tr key={entry.team.id} className={`border-b border-white/6 last:border-0 ${rankClass}`}>
...
function BestThirdRow({ entry }: { entry: TeamStanding }) {
  return (
    <tr className={`border-b border-white/6 last:border-0 ${entry.status === "third_qualified" ? "wc-rank-third" : "bg-[rgba(255,92,130,0.08)]"}`}>
      <td className={`ps-4 pe-2 py-3 font-semibold ${entry.status === "third_qualified" ? "text-wc-fg2" : "text-wc-danger"}`}>
        {entry.rank}
      </td>
...
function StatusPill({ status }: { status: TeamStanding["status"] }) {
  ...
  return (
    <span className={`mt-1 inline-flex rounded-full px-2 py-0.5 text-[10px] font-semibold ${meta.className}`}>
      {meta.label}
    </span>
  );
}
