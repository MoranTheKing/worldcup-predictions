...
- The current scoring formula is `odds base points + stage points + exact-hit points`, then Joker x2 if active. A direction hit has `0` hit points; exact score adds a stage-specific static add-on, for example `+2` in the group stage.
