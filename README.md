...
- Match-event chips now include the team logo: scoring rows show the logo beside the score, and non-scoring rows show the logo instead of a text team-name chip.
...
- If `/player-stats/` returns no rows for a BSD event, the `שחקנים בולטים` panel now builds a fallback from the full BSD event payload: official lineup ratings/minutes, goals and assists from incidents, cards, shotmap shots/xG and goalkeeper goals conceded.
